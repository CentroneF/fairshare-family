...
Refactor the authenticated dashboard into a current-month workspace with a clear month/year heading, balance, and expense list in that order. Replace the always-visible creation form with responsive entry points: a desktop dialog and a dedicated mobile page, while preserving background submissions, expense validation, settlement locks, and existing edit/review flows.

## Current State Analysis

The dashboard parses an optional `?month=` parameter and passes it to `ExpenseWorkspace` in both family states. The workspace currently presents a month selector, inline creation form, monthly balance, then expense list. Its client refresh function replaces only balance and list markup after mutations.

`CreateExpenseForm` is coupled to that dashboard refresh function through global IDs and selectors. It cannot be safely mounted in both a desktop dialog and a standalone mobile page without scoping its DOM lookup and making success behavior explicit. Balance data is intentionally absent until a second parent joins, and expense/settlement mutations already rely on API validation and a `month` form field.
...
New expenses can only be dated from the first day of the displayed current month through today. Desktop submissions update the balance/list in place, close the dialog, and restore focus to Add Expense. Mobile submissions are sent in the background and then navigate to the refreshed dashboard. One-parent families see an explanatory unavailable-balance card rather than misleading zero values.

### Key Discoveries:

- `src/pages/dashboard.astro:25-30` currently derives a selected month from the URL; `src/components/expenses/ExpenseWorkspace.astro:27-81` owns the layout that must change.
- `src/components/expenses/CreateExpenseForm.astro:85-157` prevents normal form navigation and assumes `window.refreshExpenseWorkspace`; its fixed IDs prevent duplicate desktop/mobile instances.
...
- Adding previous/next month controls or preserving hidden `?month=` navigation on the dashboard; past reports remain in Report history.
...
Make the dashboard’s visible month the current UTC month at the page boundary, then make `ExpenseWorkspace` the single current-month composition point. Extract the create form’s presentation and submission lifecycle enough to render it in either a dialog-backed dashboard flow or a standalone protected route without duplicate IDs. Keep the existing create API and progressive redirect fallback, but add a server-side invariant that a creation date belongs to the submitted dashboard month.

## Critical Implementation Details

The create form must scope all element lookups to its own rendered instance and receive an explicit post-success behavior. Do not render desktop and mobile form instances simultaneously: use a desktop dialog plus a mobile route so each submission handler has one unambiguous target. The desktop success sequence is refresh workspace → close dialog → restore trigger focus; the mobile sequence is successful background response → navigate to `/dashboard`.
...
Complete the responsive completion flows and verify that existing dashboard mutations remain reliable after the layout and form changes.
...
- A mobile create submission navigates to the current dashboard and shows the new expense and success feedback.
...
5. Attempt prior-month and future dates, trigger a server error, and confirm validation/error recovery.
6. Exercise an existing edit/review/settlement mutation and confirm the refreshed balance/list remains correct.

## Performance Considerations

The existing partial dashboard refresh is retained: successful desktop mutations fetch and replace only balance and expense-list containers. The mobile form redirects after its background response, avoiding duplicate workspace markup on small screens.
...
- [ ] 3.1 Run `npm test`, `npm run lint`, and `npm run build`

#### Manual

- [ ] 3.2 Verify desktop creation updates the workspace and restores trigger focus
- [ ] 3.3 Verify mobile creation returns to the refreshed dashboard with success feedback
- [ ] 3.4 Verify creation error recovery and existing mutation refresh behavior
