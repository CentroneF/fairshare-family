...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: pain category
      decision: Coordination overhead and conflict reduction between co-parents.
    - topic: primary persona scope
      decision: Separated or divorced co-parents.
    - topic: status quo gap
      decision: Structured form entry and automatic monthly reports are easier than maintaining a shared spreadsheet.
    - topic: authentication strategy
      decision: Each parent logs in with an email address and password.
    - topic: family membership and roles
      decision: One parent creates a family and invites the other; both parents have equal permissions.
    - topic: expense approval and deletion permissions
      decision: Creating or editing an expense requires approval from the other parent; a parent may delete only expenses they entered, without approval.
    - topic: MVP scope and timeline
      decision: Keep the complete multi-user reporting and settlement flow with a five-week target at roughly two to three after-hours hours per day and no hard deadline.
    - topic: settlement authorization
      decision: A monthly report can be marked SETTLED only with both parents' agreement.
    - topic: functional requirement priorities
      decision: The primary flow is must-have; recurring monthly expenses are nice-to-have.
    - topic: monthly settlement calculation and gate
      decision: Approved expenses are split 50/50; a month cannot settle while pending or declined expenses remain, and declined expenses must be edited and re-approved or deleted by their creator.
    - topic: monetary rounding
      decision: Use exact expense amounts and round only the final amount owed to the nearest whole unit, with .50 rounded upward.
    - topic: product surface
      decision: Responsive web app installable as a PWA on Android.
    - topic: target scale
      decision: Dozens to 100 people; the MVP keeps each family independent with exactly two equal co-parents.
    - topic: MVP scope exclusions
      decision: Fixed 50/50 split, exactly two co-parents and one family per account, no in-app money transfer, and no native mobile app beyond the Android-installable PWA.
  frs_drafted: 14
...
# General Idea
The application should help parents (divorced or not) to share monthly child costs and see on the end of the month how much money a parent owns to the other.

Each parent can add an expenses with a description and a type, and the other parent can approve it or not. 

Each month a report is generated informing both parents of major costs and who and how much money should be exchanged.
...
Acknowledged on 2026-07-14: 5-week MVP requires sustained dedication; user accepted.

## Quality cross-check

Accepted on 2026-07-14 with no quality-gate gaps.
