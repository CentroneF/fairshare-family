...
| ---------------------------- | -------------------------------- | ------------------------ | --------------------------------------------------------------------------------------------- |
| Vitest, lint, and build      | local + PR CI                    | required                 | financial-state regressions, syntax/static-rule drift, and Astro/TypeScript integration drift |
| database integration (pgTAP) | local, for migration/RLS changes | required when applicable | migrations, grants, RLS, RPCs, and persisted authorization effects                            |
...
- Run the fast local pre-PR gate with `npm run verify`. It runs the full Vitest
  suite, ESLint, and the production build in that order. `npm test` remains the
  focused standalone command when faster feedback is useful.
- Pull requests targeting `main` run the same `npm run verify` gate in GitHub
  Actions after dependencies are installed and Astro is synchronized. This
  repository does not configure whether that check is required to merge;
  branch-protection policy is external.
- For a migration or RLS change, start local Supabase (Docker required) with
  `npx supabase start`, then run `npx supabase test db`. This pgTAP gate is
  required for those changes but is deliberately not part of generic PR CI;
  do not use `supabase db reset` during normal development.
