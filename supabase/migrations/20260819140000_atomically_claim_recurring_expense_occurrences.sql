alter table public.recurring_expense_occurrences
  drop constraint recurring_expense_occurrences_expense_id_fkey;
...
begin
  ...
  loop
    ...
    insert into public.recurring_expense_occurrences (
      recurring_expense_id, occurrence_month, outcome, expense_id
    ) values (
      template_record.id, current_month, 'created', created_expense_id
    ) on conflict (recurring_expense_id, occurrence_month) do nothing
    returning expense_id into created_expense_id;

    if created_expense_id is not null then
      insert into public.expenses (id, family_id, child_id, payer_id, description, expense_date, amount_pln)
      values (
        created_expense_id,
