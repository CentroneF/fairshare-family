create table public.recurring_expense_revisions (
...
create index recurring_expense_revisions_effective_idx
  on public.recurring_expense_revisions (recurring_expense_id, effective_from desc);

alter table public.recurring_expense_revisions enable row level security;
alter table public.recurring_expense_revisions force row level security;

create policy "active parents can view recurring expense revisions"
on public.recurring_expense_revisions for select to authenticated
using (
  exists (
    select 1
    from public.recurring_expenses
    where recurring_expenses.id = recurring_expense_revisions.recurring_expense_id
      and public.is_active_family_member(recurring_expenses.family_id)
  )
);

grant select on public.recurring_expense_revisions to authenticated;
